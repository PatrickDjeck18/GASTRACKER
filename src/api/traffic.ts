...
const BASE_TRAFFIC = 'https://api.tomtom.com/traffic/services/4';
const BASE_INCIDENTS = 'https://api.tomtom.com/traffic/services/5';
...
export async function getTrafficFlow(
  ...
  try {
    const url = `${BASE_TRAFFIC}/flowSegmentData/absolute/${zoom}/json`;
...
export async function getTrafficIncidents(
  ...
  try {
    const delta = radiusKm / 111; // approximate degree offset
    const bbox = `${lon - delta},${lat - delta},${lon + delta},${lat + delta}`;
    const url = `${BASE_INCIDENTS}/incidentDetails`;
