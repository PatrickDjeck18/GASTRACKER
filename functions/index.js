...
exports.geminiFuelPrices = onRequest(GEMINI_OPTS, (req, res) => {
  return withCors(req, res, async () => {
    try {
      ...
      const brandHint = brand ? ` (${brand})` : '';
      const currencyUpper = (currencyCode || 'EUR').toUpperCase();
      const prompt = [
        `You are a fuel price assistant. Find current fuel prices for this gas station:`,
        ...
        `{"currency":"${currencyUpper}","prices":[{"fuelType":"Diesel","price":1.65},{"fuelType":"Regular","price":1.55}],"attribution":"Source description here"}`,
        ``,
        `Common fuelType values: "Diesel", "Regular", "Premium", "Midgrade", "E10", "E85", "LPG", "CNG", "Super E5", "Super E10".`,
        `Only include fuel types that are actually sold at or typical for this location.`,
      ].join('\n');
      ...
      for (const modelName of MODEL_CHAIN) {
        for (let attempt = 1; attempt <= MAX_RETRIES; attempt++) {
          try {
            ...
            const currency = ((parsed.currency || currencyUpper) + '').toUpperCase();
            ...
            const grounded = prices.length > 0;
            return res.json({ prices, currency, grounded, attribution: parsed.attribution || 'Gemini Search' });

          } catch (err) {
            lastError = err;
            const is429 = err?.message?.includes('429') || err?.status === 429;
            const isRetriable = is429 || err?.message?.includes('503') || err?.message?.includes('500');

            if (is429) {
              // Rate limited on this model — break inner loop, try next model
              logger.warn(`[geminiFuelPrices] ${modelName} rate limited (attempt ${attempt}), trying fallback`);
              break;
            }

            if (isRetriable && attempt < MAX_RETRIES) {
              const delay = BASE_DELAY_MS * Math.pow(2, attempt - 1);
              logger.warn(`[geminiFuelPrices] ${modelName} error (attempt ${attempt}), retrying in ${delay}ms: ${err.message}`);
              await new Promise(r => setTimeout(r, delay));
            } else {
              // Non-retriable or exhausted retries on this model
              break;
            }
          }
        }
